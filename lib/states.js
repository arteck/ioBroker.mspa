...
const {STATE_DEFS, CONSTANTS} = require('./constants');
const {resolveWarningCode} = require('./notificationTexts');
...
async function createStates(adapter) {
    ...
    const obsolete = ['device.deviceUuid', 'status.uvc_expiry_date', 'status.heat_target_temp_reached', // type changed from number → string
    ];
    ...
    for (const channel of channels) {
        await adapter.setObjectNotExistsAsync(channel, {
            type: 'channel', common: {name: channel}, native: {},
        });
    }
    ...
    await adapter.setObjectNotExistsAsync('status.time_windows_json', {
        type: 'state', common: {
            name: 'Configured time windows (JSON) – writable: changes are saved back to adapter config',
            type: 'string',
            role: 'json',
            read: true,
            write: true,
    ...
    // Ensure write:true is applied even when the object already existed
    {
        const existing = await adapter.getObjectAsync('status.time_windows_json');
        if (existing && existing.common && existing.common.write === false) {
            existing.common.write = true;
            existing.common.name = 'Configured time windows (JSON) – writable: changes are saved back to adapter config';
            await adapter.setObjectAsync('status.time_windows_json', existing);
        }
    }
    ...
    await adapter.setObjectNotExistsAsync('control.uvc_ensure_skip_date', {
        type: 'state', common: {
            name: 'Date when uvc_ensure_skip_today was set (YYYY-MM-DD)',
            type: 'string',
            role: 'text',
            read: true,
            write: false,
...
async function updateDeviceInfo(adapter) {
    ...
    const fields = [['device.model', api.model], ['device.series', api.series], ['device.softwareVersion', api.softwareVersion], ['device.wifiVersion', api.wifiVersion], ['device.mcuVersion', api.mcuVersion], ['device.serialNumber', api.serialNumber], ['device.alias', api.deviceAlias], ['device.macAddress', api.macAddress], ['device.productId', api.productId], ['device.productTubPk', api.productTubPk], ['device.serviceRegion', api.serviceRegion], ['device.activateIp', api.activateIp], ['device.bindingTime', api.bindingTime], ['device.activateTime', api.activateTime],];
    for (const [id, val] of fields) {
        await adapter.setStateAsync(id, {val: val || '', ack: true});
    }
    if (api.bindingRole !== null && api.bindingRole !== undefined) {
        await adapter.setStateAsync('device.bindingRole', {val: api.bindingRole, ack: true});
    }
    if (api.isCloudActivated !== null && api.isCloudActivated !== undefined) {
        await adapter.setStateAsync('device.isCloudActivated', {val: api.isCloudActivated === 1, ack: true});
    }
    if (api.productPicUrl) {
        await adapter.setStateAsync('device.pictureUrl', {val: api.productPicUrl, ack: true});
    }
}
...
async function publishStatus(adapter, data) {
    ...
    if (data.warning !== undefined) {
        await set('status.warning', resolveWarningCode(data.warning, adapter));
    }

    const setCtrl = async (id, val) => {
        if (val !== undefined && val !== null) {
            adapter.setStateChangedAsync(id, val, true);
        }
    };
    ...
    // ── Re-arm _adapterCommanded while any automation is active ──────────────
    // After _scheduleCommandedReset sets a feature to null, the detection would go
    // blind. Re-arm from the device state so it stays effective the whole session.
    if (adapter._pvActive || (Array.isArray(adapter._timeWindowActive) && adapter._timeWindowActive.some(v => v)) || adapter._winterFrostActive) {
        const rearmFeatures = [{key: 'heater', val: data.heater === 'on'}, {
            key: 'filter', val: data.filter === 'on'
        }, {key: 'uvc', val: data.uvc === 'on'}, {key: 'bubble', val: data.bubble === 'on'}, {
            key: 'ozone', val: data.ozone === 'on'
        },];
        for (const {key, val} of rearmFeatures) {
            if (adapter._adapterCommanded[key] === null) {
                adapter._adapterCommanded[key] = val;
                adapter.log.debug(`_adapterCommanded.${key} re-armed from device state (automation active)`);
            }
        }
    }

    // ── External app-change detection ────────────────────────────────────────
    const cmdGraceMs = CONSTANTS.CMD_GRACE_PERIOD_MS;
    ...
    // FIX: Detection auch bei winterModeActive/pvActive aktiv, nicht nur seasonEnabled.
    // Im Winter ist seasonEnabled typischerweise false, Frostschutz aber aktiv.
    const automationActive = adapter._seasonEnabled || adapter._winterModeActive || adapter._pvActive;
    if (!adapter._manualOverride && automationActive && !inCmdGrace) {
        const autoOverrideDuration = adapter.config.app_change_override_min ?? CONSTANTS.APP_CHANGE_OVERRIDE_MIN_DEFAULT;
        const checks = [{key: 'heater', deviceVal: data.heater === 'on'}, {
            key: 'filter', deviceVal: data.filter === 'on'
        }, {key: 'bubble', deviceVal: data.bubble === 'on'}, {key: 'uvc', deviceVal: data.uvc === 'on'}, {
            key: 'ozone', deviceVal: data.ozone === 'on'
        }, {key: 'jet', deviceVal: data.jet === 'on'},];
        if (data.heater === 'on' && adapter._adapterCommanded.target_temperature !== null) {
            ...
            if (diff > CONSTANTS.TEMP_DIFF_THRESHOLD_C) {
        ...
        for (const {key, deviceVal} of checks) {
            ...
            if (mismatch) {
                ...
                await notificationHelper.send(notificationHelper.format('appChangeDetected', {
                    key, duration: autoOverrideDuration
                }));
    ...
    if (adapter._pvActive && !adapter._pvStageTimer && !adapter._pvDeactivateTimer && data.heat_state === 4 && adapter._pvManagedFeatures.heater === false && (adapter._pvManagedFeatures.filter || adapter._pvManagedFeatures.uvc)) {
    ...
    // uvc_hours_remaining: update every poll while UVC is ON so it stays in sync
    // with uvc_hours_used (previously only updated on UVC-OFF and at startup).
    if (uvcIsOn) {
        await adapter.computeUvcExpiry();
    }
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // isHeating: heat_state 2 = actively heating (firmware heating up),
    //            heat_state 3 = actively heating (alternative firmware variant).
    //            Fallback: heater=on is used when heat_state is not reported by the device.
    //            Previously only === 3 was checked → tracker reset every poll when device
    //            reports heat_state=2, so rate was never computed.
    const isHeating = [2, 3].includes(data.heat_state) || data.heater === 'on';
    ...
    // isNotHeating: only track cooling when heater is definitively OFF.
    // If heat_state is undefined (device doesn't report it), fall back to heater flag.
    // Previously: ![2,3].includes(undefined) = true → cool tracker ran while heating.
    ...
    // ── ETA bis Zieltemperatur (hh:mm) ────────────────────────────────────────
    {
        ...
        // Konsistent mit isHeating weiter oben: OR-Verknüpfung, damit Geräte
        // ohne heat_state-Feld (nur heater=on) trotzdem eine ETA bekommen.
        ...
        if (isHeatingNow && Number.isFinite(target) && Number.isFinite(current) && target > current && rate > 0) {
            ...
            if (!Number.isFinite(etaHours) || etaHours < 0) {
                etaHours = 0;
            } else if (etaHours > CONSTANTS.MAX_ETA_HOURS) {
                etaHours = CONSTANTS.MAX_ETA_HOURS;
            }
        }
...
module.exports = {
    createStates, createDynamicStates, updateDeviceInfo, setStatusCheck, publishStatus, // Helpers (exported for testability)
    buildCommon, ensureState,
};

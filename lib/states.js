...
async function publishStatus(adapter, data) {
    ...
    if (adapter._seasonEnabled || adapter._winterModeActive || adapter._pvActive || (Array.isArray(adapter._timeWindowActive) && adapter._timeWindowActive.some(v => v)) || adapter._winterFrostActive) {
        const rearmFeatures = [{key: 'heater', val: data.heater === 'on'}, {
            key: 'filter', val: data.filter === 'on'
        }, {key: 'uvc', val: data.uvc === 'on'}, {

...
async function publishStatus(adapter, data) {
 ...
 continue; 
    ...
    if (uvcIsOn && adapter._uvcOnSince === null) {
        ...
        await adapter.setStateAsync('status.uvc_hours_used', { val: Math.round(adapter._uvcHoursUsed * 100) / 100, ack: true });
    ...
    if (filterIsOn && adapter._filterOnSince === null) {
        ...
        await adapter.setStateAsync('control.filter_running', { val: Math.round(adapter._filterHoursUsed * 100) / 100, ack: true });
    }

...
async function doPoll(adapter) {
    ...
    try {
        try {
            ...
            adapter._failedPollCount++;
            adapter.log.error(`MSpa poll error (${adapter._consecutiveErrors}): ${err.message}`);
            adapter.setState('info.connection', false, true);

            if (adapter._failedPollCount === 5) {
                adapter.log.info('MSpa poll failed 5 times consecutively – setting status.is_online to false');
                adapter.setState('status.is_online', false, true);
            }
            ...
            if (adapter._consecutiveErrors <= adapter._maxReconnectTries) {
                ...
                adapter._consecutiveErrors = 0;
                // _failedPollCount is NOT reset here – it continues counting
                // so that is_online=false is set after 5 consecutive failures.
            }
        }

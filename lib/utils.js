...
function transformStatus(raw) {
    ...
    const structured = new Set(['water_temperature', 'temperature_setting', 'heater_state', 'filter_state', 'bubble_state', 'jet_state', 'ozone_state', 'uvc_state', 'bubble_level', 'fault',]);
...
class RateTracker {
    ...
    constructor({min, max, emaAlpha = 0.25, minSampleMinutes = 3}) {
        // emaAlpha 0.25 and minSampleMinutes 3 are heuristic defaults – kept inline
        this.MIN_RATE = min;
        this.MAX_RATE = max;
        this.EMA_ALPHA = emaAlpha;
        this.MIN_SAMPLE_MS = minSampleMinutes * CONSTANTS.MS_PER_MINUTE;
        this._lastTemp = null;
        this._lastTime = null;
    ...
    update(temp, active, risingExpected) {
        ...
        if (temp === this._lastTemp) {
            ...
            if (elapsedMs > CONSTANTS.MAX_SAMPLE_AGE_MS) {
                this._lastTemp = temp;
                this._lastTime = Date.now();
            }
            return this.computedRate;
 ...
 * `adapter._strayTimers` so onUnload can clear it.
 *
 * @param {object}     adapter  – adapter instance (for _strayTimers and log)
 * @param {Function}   fn       – callback
 * @param {number}     ms       – delay in milliseconds
 * @returns {NodeJS.Timeout}
 */
function setStray(adapter, fn, ms) {
    const t = setTimeout(() => {
        adapter._strayTimers.delete(t);
        try {
            fn();
        } catch (e) {
            adapter.log.error(`stray timer cb: ${e.message}`);
        }
    }, ms);
    adapter._strayTimers.add(t);
    return t;
}
...
/**
 * Promise-based delay with stray timer tracking.
 *
 * @param {object} adapter  – adapter instance (for _strayTimers)
 * @param {number} ms       – delay in milliseconds
...
module.exports = {transformStatus, RateTracker, setStray, todayStr, sleep};

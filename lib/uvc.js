    ...
    if (!adapter._uvcEnsureActive) {
        try {
            ...
            if (uvcRunning && uvcRunning.val) {
                const ownedByOther = adapter._timeWindowActive.some(v => v);
                if (!ownedByOther) {
    ...
    if (!adapter._uvcEnsureActive) {
        ...
        try {
            ...
            if (!filterState || !filterState.val) {
                ...
                const filterOwnedByOther = adapter._timeWindowActive.some(v => v) || adapter._pvActive || adapter._winterFrostActive;
                adapter._uvcEnsureFilterStart = !filterOwnedByOther;
...
async function stopEnsure(adapter) {
    ...
    try {
        ...
        // An active time window may have turned on UVC independently –
        // ensure only manages what IT started.
        const uvcOwnedByOther = adapter._timeWindowActive.some(v => v) && Array.isArray(adapter.config.timeWindows) && adapter.config.timeWindows.some((w, j) => adapter._timeWindowActive[j] && w.active && w.action_uvc);

        if (uvcOwnedByOther) {
            adapter.log.debug('UVC daily ensure: minimum reached – UVC kept ON (owned by time-window)');
        } else {

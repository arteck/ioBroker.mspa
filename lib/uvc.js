...
const UVC_DAILY_RESET_HOUR = CONSTANTS.UVC_DAILY_RESET_HOUR;
...
const UVC_DAILY_STOP_HOUR = CONSTANTS.UVC_DAILY_STOP_HOUR;
...
function accumulateHours(adapter) {
    ...
    if (adapter._uvcOnSince !== null) {
        total += (Date.now() - adapter._uvcOnSince) / CONSTANTS.MS_PER_HOUR;
    }
    return total;
}
...
async function computeExpiry(adapter) {
    ...
    const ratedHours = cfg.uvc_operating_hours || CONSTANTS.UVC_RATED_HOURS_DEFAULT;
...
function initDailyEnsure(adapter) {
    const cfg = adapter.config;
    const minH = cfg.uvc_daily_min_h ?? CONSTANTS.UVC_DAILY_MIN_H_DEFAULT;
    ...
    adapter.setStray(() => {
        checkDailyMinimum(adapter).catch(e => adapter.log.error(`checkUvcDailyMinimum: ${e.message}`));
        adapter._uvcEnsureTimer = setInterval(() => checkDailyMinimum(adapter).catch(e => adapter.log.error(`checkUvcDailyMinimum: ${e.message}`)), CONSTANTS.UVC_ENSURE_INTERVAL_MS,);
    }, msToMin);
}
        ...
        if (adapter._uvcEnsureActive) {
            try {
                ...
                if (uvcState && uvcState.val) {
                    if (adapter.config.more_log_enabled) {
                        adapter.log.info('UVC daily ensure: skipped by user request – UVC läuft, wird gestoppt');
                    }
                    await stopEnsure(adapter);
                } else {
                    if (adapter.config.more_log_enabled) {
                        adapter.log.info('UVC daily ensure: skipped by user request – UVC bereits aus, kein Befehl gesendet');
                    }
    ...
    if (!adapter._uvcEnsureActive) {
        try {
            ...
            if (uvcRunning && uvcRunning.val) {
                const ownedByOther = (adapter._pvActive && adapter._pvManagedFeatures && adapter._pvManagedFeatures.uvc) || adapter._timeWindowActive.some(v => v);
                if (!ownedByOther) {
                    ...
                    if (filterRunning && filterRunning.val) {
                        const filterOwnedByOther = adapter._pvActive || adapter._timeWindowActive.some(v => v) || adapter._winterFrostActive;
    ...
    if (!adapter._uvcEnsureActive) {
        ...
        try {
            ...
            if (!filterState || !filterState.val) {
                ...
                const filterOwnedByOther = adapter._timeWindowActive.some(v => v) || adapter._pvActive || adapter._pvStageTimer !== null || adapter._winterFrostActive;
                adapter._uvcEnsureFilterStart = !filterOwnedByOther;
...
async function stopEnsure(adapter) {
    ...
    try {
        ...
        const uvcOwnedByOther = (adapter._pvActive && adapter._pvManagedFeatures && adapter._pvManagedFeatures.uvc) || (adapter._timeWindowActive.some(v => v) && Array.isArray(adapter.config.timeWindows) && adapter.config.timeWindows.some((w, j) => adapter._timeWindowActive[j] && w.active && w.action_uvc));
...
module.exports = {
    accumulateHours, getTodayHours, computeExpiry, initDailyEnsure, checkDailyMinimum, stopEnsure,
};

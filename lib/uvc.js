...
async function computeExpiry(adapter) {
    ...
    if (!raw) {
        ...
        await adapter.setStateAsync('status.uvc_hours_remaining', { val: 0, ack: true });
        return;
    }
 ...
 return; 
    ...
    if (adapter._uvcEnsureSkipToday) {
        ...
        if (!skipDate || skipDate !== today) {
            ...
            await adapter.setStateAsync('control.uvc_ensure_skip_today', false, true);
            await adapter.setStateAsync('control.uvc_ensure_skip_date',  '',    true);
        }
    }

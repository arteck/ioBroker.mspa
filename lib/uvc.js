...
// UVC does not run at night → counter is reset at 01:00 for the current day.
const UVC_DAILY_RESET_HOUR = 1;

// Hard stop for the ensure run: UVC must be OFF before this hour.
// Prevents the ensure run from overlapping with the nightly reset window.
...
async function checkDailyMinimum(adapter) {
    ...
    // Trigger logic:
    //  – Day WITH windows  → start ONLY after the last window has ended (ensureTime ignored)
    //  – Day WITHOUT windows → start after ensureTime (no window to wait for)
    const hasWindowsToday = lastWinEnd >= 0;
    const triggerReached  = hasWindowsToday ? lastWindowPassed : ensureTimeReached;

    if (!triggerReached) {
        if (hasWindowsToday) {
            const endHH = Math.floor(lastWinEnd / 60).toString().padStart(2, '0');
            const endMM = (lastWinEnd % 60).toString().padStart(2, '0');
            adapter.log.debug(`UVC daily ensure: waiting – last window ends at ${endHH}:${endMM}, not yet passed`);
        } else {

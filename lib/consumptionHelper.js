 ...
 * - day_kwh is ONLY written at 23:59 â€“ not updated continuously during the day.
...
const consumptionHelper = {
    ...
    async init(adapter) {
        ...
        if (!adapter.config.consumption_enabled) {
            adapter.log.debug('[consumption] tracking disabled â€“ skipping init');
            return;
        }

        if (!this.energyId) {
            adapter.log.warn('[consumption] consumption_enabled but no Object-ID configured â€“ tracking inactive');
            return;
        }
    ...
    // -------------------------------------------------------------------------
    scheduleDailyReset() {
        // NOTE: timers are cleared on cleanup(); no special filter needed here.
        ...
        const t = setTimeout(async () => {
            ...
            try {
                ...
                if (rawNow === null) {
                    this.adapter.log.warn('[consumption] 23:59 â€“ meter not readable, skipping daily close');
                } else {
                    ...
                    if (rawDayVal < 0) {
                        this.adapter.log.warn(`[consumption] 23:59 – meter value decreased (was ${savedNum}, now ${rawNow}) – possible meter replacement or rollover; resetting baseline`);
                    }
                    const dayVal = hasBase
                        ? Math.max(0, Math.round(rawDayVal * 1000) / 1000)
                        : 0;

                    this.adapter.log.info(`[consumption] 23:59 daily close â€“ day_kwh: ${dayVal} kWh, new baseline: ${rawNow} kWh`);
                    this.adapter.setState('consumption.day_kwh', {val: dayVal, ack: true});
                    this.adapter.setState('consumption.last_total_kwh', {val: rawNow, ack: true});
                }
            } catch (err) {
    ...
    // -------------------------------------------------------------------------
    /**
     * Called from onForeignStateChange for every subscribed foreign-state update.
     * The consumption helper uses a daily 23:59 scheduler instead of real-time tracking,
     * so this is intentionally a no-op. The signature is kept for API compatibility.
     *
     * @param {string} _id    â€“ foreign state id (unused)
     * @param {object} _state â€“ ioBroker state object (unused)
     */

    async handleStateChange(_id, _state) {
        // no-op: daily consumption is tracked via _scheduleDailyReset()
    },

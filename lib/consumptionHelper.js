...
return;
    ...
    async _update(rawNow) {
        try {
            ...
            if (this._dayStart === null) {
                ...
                // treat 0 / null / undefined as "never initialized" → use current meter value as baseline
                // this prevents day_kwh = full meter reading on very first start
                if (saved !== null && saved !== undefined && savedNum > 0) {
                    this._dayStart = savedNum;
                    this.adapter.log.debug(`[consumption] day-start restored from state: ${this._dayStart} kWh`);
                } else {
                    this._dayStart = rawNow;
                    await this.adapter.setStateAsync('consumption.last_total_kwh', { val: rawNow, ack: true });
                    this.adapter.log.info(`[consumption] first start – day-start baseline set to current meter value: ${rawNow} kWh`);
                }
            }
    ...
    async _restoreDayStart() {
        try {
            ...
            // if saved = 0 → leave _dayStart = null, will be set on first meter value
        } catch (err) {

 ...
 *   cancelAllDeactivationTimers(adapter)           – clear debounce timer/countdown
 *   stagedDeactivate(adapter, pvWindows, immediate)– switches the heater OFF (filter/UVC stay under time-window control)
 */
...
async function init(adapter) {
    ...
    if (pvActiveState && pvActiveState.val === true) {
        ...
        // Ownership der Heizung nach Neustart rekonstruieren:
        // _pvManagedFeatures wird im Konstruktor immer auf false gesetzt und nicht
        // persistiert. Ohne Rekonstruktion überspringt stagedDeactivate() den
        // Heizungs-AUS-Block (Guard: _pvManagedFeatures.heater) und die Heizung
        // liefe außerhalb des Überschuss-Fensters unbeaufsichtigt weiter.
        if (adapter._lastData && adapter._lastData.heater === 'on') {
            adapter._pvManagedFeatures.heater = true;
            adapter.log.debug('PV: restored _pvManagedFeatures.heater=true (device reports heater ON after restart)');
        }
...
async function evaluateSurplus(adapter) {
    ...
    // ── Activation ──────────────────────────────────────────────────────────
    if (shouldActivate && !adapter._pvActive) {
        await cancelAllDeactivationTimers(adapter);
    ...
    } else if (adapter._pvActive && !shouldDeactivate && adapter._pvDeactivateTimer) {
        await cancelAllDeactivationTimers(adapter);
        if (adapter.config.more_log_enabled) {
        ...
        adapter.log.debug(`PV: no action (pvActive=${adapter._pvActive}, shouldActivate=${shouldActivate}, shouldDeactivate=${shouldDeactivate}, debounce=${!!adapter._pvDeactivateTimer})`);
    }
}

/**
 * Cancel debounce timer and countdown interval.
...
/**
 * PV deactivation: switches the heater OFF.
 *
 * PV only ever manages the heater – filter and UVC are always managed by the
 * time window itself (they keep running independent of PV surplus) and are
 * therefore never switched off here. After the heater is handled, the time
 * window logic is re-evaluated so it can apply its own filter/UVC follow-up
 * rules (e.g. pump follow-up delay).
 *
 * The `immediate` parameter is currently unused (kept for API compatibility)
 * since there are no more inter-stage delays.
 *
 * @param {object}  adapter
...
module.exports = {
    init, onForeignStateChange, evaluateSurplus, cancelAllDeactivationTimers, stagedDeactivate,
};

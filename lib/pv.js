...
async function evaluateSurplus(adapter) {
    ...
    if (pvWindows.length === 0) {
        adapter.log.debug('PV: no PV-enabled time window rows – nothing to do');
        return;
    }
 ...
 adapter.enableRapidPolling(); 
    ...
    // ── Surplus gone: start debounce ─────────────────────────────────────
        ...
        await adapter.setStateAsync('computed.pv_deactivate_remaining', delayMin, true);
 ...
 clearInterval(adapter._pvDeactivateCountdownInt); 
            ...
            adapter._pvDeactivateCountdownInt = setInterval(async () => {
                ...
                await adapter.setStateAsync('computed.pv_deactivate_remaining', remaining, true);
            }, 60_000);
        }
        ...
        adapter._pvDeactivateTimer = setTimeout(async () => {
            ...
            await adapter.setStateAsync('computed.pv_deactivate_remaining', 0, true);
            if (adapter.config.more_log_enabled) {
...
async function cancelAllDeactivationTimers(adapter) {
 ...
 clearTimeout(adapter._pvDeactivateTimer);         adapter._pvDeactivateTimer        = null; 
}
    if (adapter._pvDeactivateCountdownInt) {
 clearInterval(adapter._pvDeactivateCountdownInt); adapter._pvDeactivateCountdownInt = null; 
}
    if (adapter._pvStageTimer)             {
 clearTimeout(adapter._pvStageTimer);              adapter._pvStageTimer             = null; 
}
    adapter._pvDeactivateCountdown = 0;
    await adapter.setStateAsync('computed.pv_deactivate_remaining', 0, true);
}

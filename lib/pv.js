            ...
            for (const w of pvWindows) {
                try {
                    if (w.action_heating) {
                        ...
                        if (tempToSet != null) {
                            adapter.setStray(() => {
                                adapter.sendTargetTempDirect(tempToSet, { fromAutomation: true }).catch(e =>
                    ...
                    if (w.action_uvc) {
                        // UVC needs filter running – start it if not already managed
                        if (!w.action_filter && !w.action_heating && !adapter._pvManagedFeatures.filter) {
                            await adapter.setFeature('filter', true, { fromAutomation: true });
                            adapter._pvManagedFeatures.filter = true;
                        }
...
async function reactivate(adapter, pvWindows, surplus) {
    ...
    for (const w of pvWindows) {
        try {
            ...
            if (w.action_uvc && !adapter._pvManagedFeatures.uvc) {
                // UVC needs filter – start filter if not yet managed
...
async function stagedDeactivate(adapter, pvWindows, immediate = false) {
    ...
    const runStage2 = async () => {
        ...
        const runStage3 = async () => {
            ...
            adapter.enableRapidPolling();
            // Re-evaluate time windows now that PV is no longer active
            adapter.checkTimeWindows().catch(e =>
                adapter.log.error(`PV staged shutdown: checkTimeWindows re-eval failed – ${e.message}`)
            );
        };

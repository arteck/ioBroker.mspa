        ...
        if (inWin && !wasIn) {
            try {
                ...
                if (!w.action_filter && !w.action_heating && !w.action_uvc) {
                    ...
                    // ── Jede Aktion startet unabhängig von den anderen, direkt mit dem Zeitfenster:
                    //    action_filter=true  → Filter startet sofort
                    //    action_uvc=true     → UVC startet sofort (mit Filter, siehe commands.js Auto-Start)
                    //    action_heating=true → Heizung startet
                    //        – pv_steu=true  → NUR wenn genug PV-Überschuss (evaluatePvSurplus entscheidet)
                    //        – pv_steu=false → sofort, unabhängig vom PV-Überschuss
                    if (w.action_filter) {
                    ...
                    if (w.action_heating) {
                        if (w.pv_steu) {
                            ...
                            // pv_steu=false → Heizung direkt starten (kein PV-Überschuss benötigt)
                    ...
                    if (w.action_uvc) {
                        // UVC sofort starten (action_uvc=true). Falls der Filter noch nicht läuft,
                        // startet commands.js ihn automatisch mit (Geräte-Voraussetzung für UVC).
                        // action_uvc=false → UVC-Ensure-Logik übernimmt (min. Stunden/Tag ab Startzeit)
                        adapter.log.debug(`Time control [${i + 1}]: UVC ON (window start)`);
...
async function deactivateWindow(adapter, w, i) {
    ...
    const pvHandlesHeater = w.pv_steu && adapter._pvActive;
    if (pvHandlesHeater) {
        if (adapter.config.more_log_enabled) {
    ...
    // Note: action_heating/action_uvc also require the filter (device requirement,
    // see commands.js auto-start), so both count towards "other window needs filter".
    const windows = adapter.config.timeWindows;
    const otherNeedsFilter = Array.isArray(windows) && windows.some((win, j) => j !== i && adapter._timeWindowActive[j] && win.active && (win.action_filter || win.action_heating || win.action_uvc));
    ...
    try {
        ...
        // Heizer läuft wenn action_heating=true (direkt bei pv_steu=false, sonst via PV).
        const windowStartedHeater = w.action_heating;

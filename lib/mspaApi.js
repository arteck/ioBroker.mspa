...
return '***';
    ...
    async _sendCommandLocked(desiredDict, retry = false) {
        ...
        // Auto-disable heater if filter was turned off.
        // Call _sendCommandLocked directly (bypass sendDeviceCommand lock)
        // to avoid a deadlock – we are still inside the active _cmdPromise here.
        // Preserve the filter confirmation result so statusCheck reflects
        // the filter command, not the heater follow-up.
        if (desiredDict.filter_state === 0) {
            const filterConfirmed = this._lastCommandConfirmed;
            this._log('info', 'MSpa: filter OFF – auto-disabling heater');
            await this._sendCommandLocked({ heater_state: 0 });
            this._lastCommandConfirmed = filterConfirmed;
        }

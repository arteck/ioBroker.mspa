...
describe('control.uvc_ensure_skip_today � ack + ensure stop', () => {
    ...
    async function handleUvcEnsureSkip(stateVal, opts = {}) {
        ...
        if (skip) {
            ...
            // skip only pauses the ensure scheduler – UVC running via time window
    ...
    it('does NOT turn off UVC via skip when UVC runs via time window (no synchronous getState)', async () => {
        ...
        assert.strictEqual(setFeatureCalled, false, 'setFeature must NOT be called – skip only affects ensure scheduler');
    });
...
// 25. _applyTimeWindowsJson � validate, persist to config, restart schedulers
// ---------------------------------------------------------------------------
describe('_applyTimeWindowsJson � JSON write-back to adapter config', () => {
    function makeAdapter(opts = {}) {
        const saved = {};
        ...
        a._applyTimeWindowsJson = async function (jsonStr) {
            ...
            try { parsed = JSON.parse(jsonStr); } catch (e) {
                a.log.error(`time_windows_json: invalid JSON � ${e.message}`);
                await a.publishTimeWindowsJson(); return;
            }
            if (!Array.isArray(parsed)) {
                a.log.error('time_windows_json: value must be a JSON array � ignoring');
                await a.publishTimeWindowsJson(); return;
            }
            try {
    ...
    it('rejects invalid JSON � rollback to current value', async () => {
    ...
    it('rejects non-array JSON (object) � rollback', async () => {
    ...
    it('rejects empty string � error logged', async () => {
        const { a, logged } = makeAdapter();
        await a._applyTimeWindowsJson('');
        assert.ok(logged.errors.some(m => m.includes('invalid JSON')));
    });
    it('accepts empty array [] � clears all windows', async () => {
...
// 26. heat_rate / cool_rate � isHeating flag fix
// ---------------------------------------------------------------------------
describe('heat_rate / cool_rate � isHeating flag', () => {
    /**
    ...
    it('computes heat_rate when heat_state=2 (previously treated as inactive)', () => {
        ...
        // 0.2�C in 4 min = 3.0�C/h ? at MAX boundary, should still yield a rate
        assert.notStrictEqual(heatRate, null,
            'heat_rate must NOT be null when heat_state=2 � was previously broken');
    ...
    it('does NOT accumulate cool_rate while heater is ON', () => {
        ...
        assert.strictEqual(coolRate, null,
            'cool_rate must stay null while heater is ON � was previously computed wrongly when heat_state was undefined');
    });
    it('computes cool_rate when heater is OFF and temp drops', () => {
        const { coolRate } = runTracker({
            elapsedMs: 8 * 60 * 1000,   // 8 min → 0.2°C/8min = 1.5°C/h, well within [0.01, 3.0]
